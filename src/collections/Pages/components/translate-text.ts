...
export const translatePageText = async (pageId: string) => {
 ...
 */
function processMediaBlocksInPage(obj: unknown, visited = new WeakSet()): unknown {
  ...
  if (Array.isArray(obj)) {
    return obj.map((item) => processMediaBlocksInPage(item, visited))
  }
  ...
  for (const [key, value] of Object.entries(record)) {
    ...
    if (
      ...
      processed[key] = value.map((item) => processMediaBlocksInPage(item, visited))
    } else if (typeof value === 'object') {
      processed[key] = processMediaBlocksInPage(value, visited)
    } else {
      processed[key] = value
    }
  }
...
export const createAndUpdateTranslatedPages = async (
  ...
  const originalPageResult = await payload.find({
    collection: 'pages',
    where: {
      id: {
  ...
  if (!originalPageResult.docs[0]) {
    return { error: 'Original page not found' }
  }

  const originalPage = originalPageResult.docs[0] as unknown as Record<string, unknown>
  ...
  for (const localeTranslation of translationsData) {
    ...
    const translatedPage: Record<string, unknown> = JSON.parse(JSON.stringify(originalPage))
    ...
    for (const translation of localeTranslation.translations) {
      setNestedValue(translatedPage, translation.path, translation.value)
    }

    // Process MediaBlocks in richText fields
    const processedPage = processMediaBlocksInPage(translatedPage) as Record<string, unknown>

    // Regenerate IDs for all link fields
    const pageWithNewLinkIds = regenerateLinkIds(processedPage) as Record<string, unknown>

    console.dir(pageWithNewLinkIds, { depth: null })
    ...
    try {
      ...
      const existingPageResult = await payload.find({
        ...
        where: {
          slug: {
            equals: pageWithNewLinkIds.slug as string,
          },
        },
        locale: locale,
      })
      ...
      if (existingPageResult.docs[0]) {
        ...
        await payload.update({
          ...
          data: pageWithNewLinkIds as never,
          locale: locale,
        })
