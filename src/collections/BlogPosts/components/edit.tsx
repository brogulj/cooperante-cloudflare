...
import { translateBlogPostRichText } from './translate-rich-text'
import { Button, toast } from '@payloadcms/ui'
import { translateBlogPostText } from './translate-text'
import { useState } from 'react'
import { CTABlock } from '@/blocks/CTABlock/config'

export const Translate = () => {
  const params: { segments: string[] } = useParams()
  const blogPostId = params.segments[2]
  ...
  const handleTranslate = async () => {
    ...
    toast.promise(
      async () => {
        ...
        await translateBlogPostText(blogPostId)
        await translateBlogPostRichText(blogPostId)
      },
      {
        loading: 'Translating...',
