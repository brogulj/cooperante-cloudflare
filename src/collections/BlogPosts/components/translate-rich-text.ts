...
// import { extractBlogPostTextForTranslation } from '@/lib/translationUtils'
...
export const translateBlogPostRichText = async (pageId: string) => {
  ...
  const richTextLocalizedPaths = extractRichTextLocalizedPaths(BlogPosts)
  ...
  const result = await createAndUpdateTranslatedBlogPosts(pageId, validTranslations)
...
export const createAndUpdateTranslatedBlogPosts = async (
  ...
  for (const localeTranslation of translationsData) {
    ...
    const localizedBlogPost = await payload.find({
      collection: 'blogPosts',
      where: {
        id: {
    ...
    if (!localizedBlogPost.docs[0]) {
      return { error: 'Localized page not found' }
    }

    const translatedBlogPost: Record<string, unknown> = JSON.parse(
      JSON.stringify(localizedBlogPost.docs[0]),
    )
    ...
    for (const translation of localeTranslation.translations) {
      setNestedValue(translatedBlogPost, translation.path, translation.richText)
    }
    ...
    try {
      ...
      const existingBlogPostResult = await payload.find({
        ...
        where: {
          slug: {
            equals: translatedBlogPost.slug as string,
          },
        },
        locale: locale,
      })
      ...
      if (existingBlogPostResult.docs[0]) {
        ...
        await payload.update({
          ...
          data: translatedBlogPost as never,
          locale: locale,
        })
