...
// import { extractJobAdTextForTranslation } from '@/lib/translationUtils'
...
export const translateJobAdRichText = async (pageId: string) => {
  ...
  const richTextLocalizedPaths = extractRichTextLocalizedPaths(JobAds)
  ...
  const result = await createAndUpdateTranslatedJobAds(pageId, validTranslations)
...
export const createAndUpdateTranslatedJobAds = async (
  ...
  for (const localeTranslation of translationsData) {
    ...
    const localizedJobAd = await payload.find({
      collection: 'jobAds',
      where: {
        id: {
    ...
    if (!localizedJobAd.docs[0]) {
      return { error: 'Localized page not found' }
    }

    const translatedJobAd: Record<string, unknown> = JSON.parse(
      JSON.stringify(localizedJobAd.docs[0]),
    )
    ...
    for (const translation of localeTranslation.translations) {
      setNestedValue(translatedJobAd, translation.path, translation.richText)
    }
    ...
    try {
      ...
      const existingJobAdResult = await payload.find({
        ...
        where: {
          slug: {
            equals: translatedJobAd.slug as string,
          },
        },
        locale: locale,
      })
