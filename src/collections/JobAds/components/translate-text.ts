...
export const translateJobAdText = async (jobAdId: string) => {
  const startTime = new Date()
  const translationTracking: Map<SupportedLocale, TranslationStatus> = new Map()

  const payload = await getPayload({ config })
  const jobAd = await payload.find({
    collection: 'jobAds',
    where: {
      id: {
        equals: jobAdId,
  ...
  if (!jobAd.docs[0]) {
    return { error: 'JobAd not found' }
  }

  const localizedPaths = extractLocalizedPaths(JobAds)
  const _translationPayload = extractLocalizedValues(jobAd.docs[0], localizedPaths)
  ...
  // Create and update translated jobAds in payload
  const validTranslations = translations.filter((t) => t && t.success)
  const result = await createAndUpdateTranslatedJobAds(jobAdId, validTranslations)
 ...
 */
function processMediaBlocksInJobAd(obj: unknown, visited = new WeakSet()): unknown {
  ...
  if (Array.isArray(obj)) {
    return obj.map((item) => processMediaBlocksInJobAd(item, visited))
  }
  ...
  for (const [key, value] of Object.entries(record)) {
    ...
    if (
      ...
      processed[key] = value.map((item) => processMediaBlocksInJobAd(item, visited))
    } else if (typeof value === 'object') {
      processed[key] = processMediaBlocksInJobAd(value, visited)
    } else {
      processed[key] = value
    }
  }
...
export const createAndUpdateTranslatedJobAds = async (
  originalJobAdId: string,
  translationsData: {
    locale: string
    translations: {
      path: string
  ...
  // Get the original jobAd
  const originalJobAdResult = await payload.find({
    collection: 'jobAds',
    where: {
      id: {
        equals: originalJobAdId,
      },
    },
  })

  if (!originalJobAdResult.docs[0]) {
    return { error: 'Original jobAd not found' }
  }

  const originalJobAd = originalJobAdResult.docs[0] as unknown as Record<string, unknown>

  // Create or update jobAds for each locale
  for (const localeTranslation of translationsData) {
    const locale = localeTranslation.locale as SupportedLocale

    // Create a deep copy of the original jobAd for this locale
    const translatedJobAd: Record<string, unknown> = JSON.parse(JSON.stringify(originalJobAd))

    // Remove the id and Payload metadata fields so we can create a new document
    delete translatedJobAd.id
    delete translatedJobAd.createdAt
    delete translatedJobAd.updatedAt

    // Apply translations to the jobAd
    for (const translation of localeTranslation.translations) {
      setNestedValue(translatedJobAd, translation.path, translation.value)
    }

    // Process MediaBlocks in richText fields
    const processedJobAd = processMediaBlocksInJobAd(translatedJobAd) as Record<string, unknown>

    // Regenerate IDs for all link fields
    const jobAdWithNewLinkIds = regenerateLinkIds(processedJobAd) as Record<string, unknown>

    console.dir(jobAdWithNewLinkIds, { depth: null })

    try {
      // Check if a jobAd with this slug already exists for this locale
      const existingJobAdResult = await payload.find({
        collection: 'jobAds',
        where: {
          slug: {
            equals: jobAdWithNewLinkIds.slug as string,
          },
        },
        locale: locale,
      })

      if (existingJobAdResult.docs[0]) {
        // Update existing jobAd
        await payload.update({
          collection: 'jobAds',
          id: existingJobAdResult.docs[0].id,
          data: jobAdWithNewLinkIds as never,
          locale: locale,
        })
        console.log(`Updated jobAd for locale: ${locale}`)
      } else {
        // Create new jobAd
        ...
        console.log(`Created new jobAd for locale: ${locale}`)
      }
    } catch (error) {
  ...
  return { success: true, message: 'All translated jobAds created/updated successfully' }
}
