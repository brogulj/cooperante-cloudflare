...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  await db.run(sql`CREATE TABLE \`blog_posts\` (
  	...
  	\`header_theme\` text DEFAULT 'light',
  	\`author_id\` integer,
  	\`published_at\` text,
  	\`slug\` text,
  ...
  `)
  await db.run(sql`CREATE INDEX \`blog_posts_header_image_idx\` ON \`blog_posts\` (\`header_image_id\`);`)
  await db.run(sql`CREATE INDEX \`blog_posts_author_idx\` ON \`blog_posts\` (\`author_id\`);`)
  await db.run(sql`CREATE INDEX \`blog_posts_slug_idx\` ON \`blog_posts\` (\`slug\`);`)
  ...
  await db.run(sql`CREATE INDEX \`blog_posts_rels_categories_id_idx\` ON \`blog_posts_rels\` (\`categories_id\`);`)
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  await db.run(sql`CREATE TABLE \`__new_payload_locked_documents_rels\` (
  	...
  	FOREIGN KEY (\`job_ads_id\`) REFERENCES \`job_ads\`(\`id\`) ON UPDATE no action ON DELETE cascade
  );
  `)
  await db.run(sql`INSERT INTO \`__new_payload_locked_documents_rels\`("id", "order", "parent_id", "path", "users_id", "media_id", "pages_id", "job_ads_id") SELECT "id", "order", "parent_id", "path", "users_id", "media_id", "pages_id", "job_ads_id" FROM \`payload_locked_documents_rels\`;`)
