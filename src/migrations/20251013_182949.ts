...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  await db.run(sql`CREATE INDEX \`forms_rels_blog_posts_id_idx\` ON \`forms_rels\` (\`blog_posts_id\`);`)
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  `)
  await db.run(sql`INSERT INTO \`__new_payload_locked_documents_rels\`("id", "order", "parent_id", "path", "users_id", "media_id", "pages_id", "job_ads_id", "blog_posts_id", "categories_id") SELECT "id", "order", "parent_id", "path", "users_id", "media_id", "pages_id", "job_ads_id", "blog_posts_id", "categories_id" FROM \`payload_locked_documents_rels\`;`)
